...
Ticker cmd_timeout_checker, range_ticker;
...
// This function will be regularly called by a ticker to poll the range sensors
// and store the ranges in ranges vector
void readRanges()
{
  auto r_it = ranges.begin();
  auto s_it = sensors.begin();
  for (; s_it != sensors.end() && r_it != ranges.end(); s_it++, r_it++)
  {
    VL53L0X& sensor = *s_it;
    *r_it = sensor.readRangeContinuousMillimeters();
  }
}
...
int main()
{
  ...
  cmd_timeout_checker.attach(checkForTimeout, 0.1);
  range_ticker.attach(readRanges, 0.1);
  cmd_timer.start();
  // SCAN i2c devices!
  initSensors();
