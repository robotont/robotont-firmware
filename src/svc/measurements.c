...
void measurements_init(void)
{
    ...
    pwr_mgmnt_data.stop_btn_pressed = false;
    pwr_mgmnt_data.power_sw_pressed = false;
    pwr_mgmnt_data.wall_power_present = false;
...
void measurements_processData(void)
{
    ...
    uint8_t status_byte = pwr_mgmnt_data_raw[0];
    pwr_mgmnt_data.stop_btn_pressed = (status_byte & 0x01) != 0;
    pwr_mgmnt_data.power_sw_pressed = (status_byte & 0x02) != 0;
    pwr_mgmnt_data.wall_power_present = (status_byte & 0x04) != 0;
    pwr_mgmnt_data.motor_power_enabled = (status_byte & 0x08) != 0;
    pwr_mgmnt_data.sys_power_enabled = (status_byte & 0x10) != 0;
    
    uint16_t mtr_current_reading = (pwr_mgmnt_data_raw[1] << 8) | pwr_mgmnt_data_raw[2];
    ...
    uint16_t bat_voltage_reading = (pwr_mgmnt_data_raw[7] << 8) | pwr_mgmnt_data_raw[8];
    pwr_mgmnt_data.bat_voltage = (((bat_voltage_reading * 3.3) / 1024) * 118) / 18; // VOLTS

    pwr_mgmnt_data.bat_pack_voltage = ((pwr_mgmnt_data_raw[9] << 8) | pwr_mgmnt_data_raw[10]) / 1000.0f; // VOLTS

    for (int i = 0; i < 5; i++) {
        pwr_mgmnt_data.bat_cell_voltages[i] = ((pwr_mgmnt_data_raw[11 + i*2] << 8) | pwr_mgmnt_data_raw[12 + i*2]) / 1000.0f; // VOLTS
    }
